...
<svg height="{starConfig.size}" viewBox="0 -10 187.673 179.503" width="{starConfig.size}"
     xmlns="http://www.w3.org/2000/svg">
    {#if fillPercent < 1 && fillPercent > 0}
        <defs>
            <linearGradient id="linear-gradient-{id}" x1="0%" y1="0%" x2="100%" y2="0%">
                <stop offset="0%" style="stop-color:{starConfig.filledColor};stop-opacity:1" />
                <stop offset="{fillPercent * 100}%" style="stop-color:{starConfig.filledColor};stop-opacity:1"/>
                <stop offset="{fillPercent * 100}%" style="stop-color:{starConfig.unfilledColor};stop-opacity:1" />
            </linearGradient>
        </defs>
    {/if}
    <path
            ...
            fill={fillPercent === 1 ? starConfig.filledColor : fillPercent === 0 ? starConfig.unfilledColor : `url(#linear-gradient-${id})`}
            opacity="{readonly ? .8 : 1}"
    />
<!--            stroke={fillPercent > 0 ? starConfig.filledColor : starConfig.unfilledColor}-->
</svg>